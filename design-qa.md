...
旧 `?design=inspection` 模拟账户、模拟媒体数据和山景图片已移除；下方相关记录只代表历史验收。当前视觉规范统一在根 design.md，docs/design/frontend-visual-system.md 转发文档已删除。浏览器验收需要使用真实会话或隔离的网络响应测试数据。
...
- boundary regression：桌面、移动与目录业务测试均断言导航根节点的 `data-slot="navigation-menu"`，当前页仍保留 `aria-current="page"`，移动链接激活后仍由 Sheet 关闭。
...
- 候选生产构建运行于独立 `127.0.0.1:8127`；上游固定为不可用测试端口，agent-browser 仅注入合成账号和任务，不访问生产 API。此项验证 UI 分支，不是平台下载验收。
- 本地 `browser_import` 失败：详情与历史列表显示“返回首页重新导入”，无远程重试；鼠标点击与列表键盘 focus/Enter 都返回首页。远程来源详情仍显示“重新下载”。三种本地终态另有组件回归。
- 详情 390×844、1280×900，浅/深主题同状态检查；截图无横向溢出。首次 axe 发现无封面文字 4.35:1；修正为既有语义色的 70% 不透明度后，四次 WCAG 2 A/AA 检查均为 0 violations / 0 incomplete。截图位于本次临时 QA 输出，不进入产品素材。
- 合成视频传输文件被选择后，创建接口返回既有 `verifying` 资源，页面跳到返回的下载任务；仅一次 media-imports POST，无 upload-session/complete/存储 PUT。没有真实视频校验或制品生成，不据此关闭完整上传端到端门禁。
- 本增量沿用现有网格、排版、控件和主题，未进行全站所有状态重验；没有本切片剩余 P0/P1/P2 视觉差异。

final result: passed（仅上述合成下载恢复交互增量；不包含平台、重启、换机或真机验收）

## 2026-09-13 链接入口简化

- 用户现有已登录 Chrome，真实 localhost:8101 生产镜像，无合成账号/响应：输入框与解析按钮保留，删除策略选择、运行说明与结果策略标签；导航和三个既有内容入口不变。
- 1280×900 桌面与 390×844 移动视口检查；页面内容宽度分别为 1265/375px（浏览器滚动条占 15px），scrollWidth 等于 clientWidth。检查浅色初始/输入、深色移动错误状态，主题切换后恢复原浅色及默认视口。
- 空输入提示、输入清除错误、键盘 Tab 到解析按钮并 Enter 提交、解析中禁用、真实上游失败均已验证。移动导航 Sheet 打开/Escape 关闭，待关闭完成后焦点返回“打开导航菜单”。没有策略 combobox，没有本增量剩余 P0/P1/P2 视觉差异。
- agent-browser 检查真实未登录 8101 页面，WCAG 2 A/AA：0 violations / 0 incomplete；该 axe 结果不代表已登录全站无障碍通过。成功与空格式分支保留组件测试覆盖，原 YouTube 未返回格式，不能以本次 UI 检查替代真实下载成功验收。

final result: passed（仅链接入口简化的视觉交互增量；平台下载仍未通过）

## 2026-09-13 封面等宽、自适应高度

- 用户最终确认：同一布局统一封面宽度，高度按素材原始比例自适应，等比铺满宽度，不拉伸、不裁切、不补边凑等高。Web 复用 Next Image 的自然高度、Vidstack 原生 `aspectRatio="auto"` 与默认布局；移除自定义播放器 CSS 覆盖。Flutter 使用原生 Image、StreamBuilder/AspectRatio 与媒体运行时真实尺寸，不保留固定比例 MediaFrame 封装，也不新增依赖。
- agent-browser 在重新构建后的 localhost:8101 上，以隔离会话注入 160×90、90×160、100×100 三张合成封面，未写入数据库。1280px 桌面下宽度均为 128px，高度分别为 72、227.55、128px；390px 移动下宽度均为 96px，高度分别为 54、170.66、96px。明暗主题均无横向溢出，图片边界等于容器边界；目视复核圆形未变形、四周标记完整。截图位于 `/tmp/framefetch-cover-audit-20260913/screenshots/adaptive-{1280,390}-{light,dark}.png`。
- Flutter 三种真实 PNG 解码回归覆盖 112px 与 320px 宽度，断言源图完整参与绘制、目标图铺满宽度且高度按原始比例变化。测试先在固定横框实现上复现竖图/方图失败，再在原生自然高度实现上通过。iPhone 17 Pro Max 重建运行并热更新后，真实历史列表、1080×1440（3:4）详情目视确认宽度铺满、高度自适应。
- Vidstack 原生配置回归：临时 8129 同源代理转发 8101 的生产页面/资源，仅额外提供本地生成的三种 MP4 夹具；API 响应在隔离浏览器内注入，未改安全策略或业务数据。160×90、90×160、100×100 视频均解码成功；1280px 视口播放器宽度均 719px，高度依次 405/1279/719px；390px 视口宽度均 356px，高度依次 200/633/356px。视频与 poster 尺寸一致且均 contain，使用 `aspectRatio="auto"` 而非自定义样式覆盖。此项验证解码/布局，不冒充真实平台下载与持续播放验收。
- 工程检查：Web lint、63 文件/286 项全量测试通过；切换 Vidstack 原生属性后再过 lint 和 22 项相关测试、Docker production build，并重建前端容器。Flutter 全量 208 项通过；占位改用现有间距 token 后再过 analyze 和 9 项媒体测试。两个仓库 diff whitespace 检查通过。
