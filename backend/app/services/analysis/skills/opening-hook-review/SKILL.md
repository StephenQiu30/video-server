...
metadata:
  ...
  video-server-default-prompt: 完整观察全片并区分真实编辑边界与连续视觉节拍，重点审查开场 3、5、15 秒的主体锚点、内容承诺、文字可读性、画面进展和正文衔接。
...
1. 先覆盖完整时间轴并建立同时包含真实编辑边界与连续视觉节拍的 `shots` 和连续 `scenes`；不得因聚焦开场而省略后续内容，也不得因开场是长镜头就把所有进展压成一镜。
