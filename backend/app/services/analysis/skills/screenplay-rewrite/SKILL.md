...
按源场景顺序改写中文或英文剧本。目标语言由任务参数确定：目标与源语言不同时做忠实本地化，相同时做表达润色。改写结果是新的不可变文本版本候选，不覆盖原稿，也不自动修改资产、镜头、主选或 coverage 决策。不得改变故事事实、场景数量或角色身份。
...
4. 每个 chunk 只对应一个源场景及连续 part，携带受控流程给出的 `source_sha256`；不自行改写标识或哈希。
