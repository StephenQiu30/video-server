...
from app.services.analysis.rules.errors import InvalidAnalysisTransition
from app.services.analysis.rules.job_rules import (
    AnalysisJobRules,
...
class AnalysisJob(AnalysisJobRules):
    ...
    def schedule_retry(
        ...
        from app.services.analysis.rules.job_failures import schedule_retry

        schedule_retry(self, owner, error_code, now, retry_at)

    def release_retry(self, now: datetime) -> None:
        from app.services.analysis.rules.job_failures import release_retry
    ...
    def recover_expired_lease(
        ...
        from app.services.analysis.rules.job_failures import recover_expired_lease
    ...
    def fail(
        ...
        from app.services.analysis.rules.job_failures import fail

        fail(self, error_code, now, owner)

    def cancel(self, now: datetime) -> None:
        from app.services.analysis.rules.job_failures import cancel
