...
def test_operator_requires_the_shared_provider_cookie_agent(tmp_path: Path) -> None:
    ...
    settings = RunnerSettings(
        ...
        runner_provider_cookie_sync_root=bridge,
        runner_max_active_tasks=1,
    )

    assert settings.runner_provider_cookie_sync_root == bridge.resolve()


def test_anonymous_mode_cannot_enable_cookie_sync(tmp_path: Path) -> None:
    with pytest.raises(ValidationError, match="restricted to an operator runner"):
        RunnerSettings(
            runner_hmac_secret=SECRET,
            runner_egress_proxy="http://egress-proxy:3128",
            runner_workspace_root=tmp_path / "work",
...
def test_other_operator_provider_uses_the_same_cookie_agent_protocol(
    tmp_path: Path,
) -> None:
    settings = RunnerSettings(
        runner_hmac_secret=SECRET,
    ...
    assert settings.runner_operator_session_versions == {"x": "browser"}


def test_operator_rejects_a_non_policy_session_version(tmp_path: Path) -> None:
    with pytest.raises(ValidationError, match="Input should be 'browser'"):
        RunnerSettings(
            runner_hmac_secret=SECRET,
            runner_egress_proxy="http://egress-proxy:3128",
            runner_workspace_root=tmp_path / "work",
            runner_access_mode="operator_managed",
            runner_operator_session_versions={"x": "manual"},
            runner_operator_account_baseline_attested=True,
            runner_provider_session_temp_root=tmp_path / "sessions",
            runner_provider_cookie_sync_root=tmp_path / "bridge",
            runner_max_active_tasks=1,
...
def test_cookie_sync_root_cannot_be_inside_runner_workspace(tmp_path: Path) -> None:
    with pytest.raises(ValidationError, match="cannot be in the workspace"):
        RunnerSettings(
            ...
            runner_operator_account_baseline_attested=True,
            runner_provider_session_temp_root=tmp_path / "sessions",
            runner_provider_cookie_sync_root=tmp_path / "work/bridge",
            runner_max_active_tasks=1,
        )
...
def test_cookie_sync_root_cannot_traverse_into_runner_workspace(
    ...
    with pytest.raises(ValidationError, match="cannot be in the workspace"):
        RunnerSettings(
            ...
            runner_operator_account_baseline_attested=True,
            runner_provider_session_temp_root=tmp_path / "sessions",
            runner_provider_cookie_sync_root=tmp_path / "outside/../work/bridge",
            runner_max_active_tasks=1,
        )
