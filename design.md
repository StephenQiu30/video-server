# FrameFetch 界面设计规范

本文件是 FrameFetch Web 界面唯一的视觉设计标准。2026-09-24 起，产品采用黑白中性色主题和 shadcn/ui 官方组件样式。用户确认的这一方向取代此前的彩色渐变、胶囊按钮、额外阴影和手工改写组件源码的方案。历史版本保留在 Git 记录中。

## 基础技术与来源

- Web 使用 Next.js、Tailwind CSS v4、shadcn/ui 的 `radix-nova` 样式与 Radix primitives。以 `frontend/components.json` 的 `style`、`base`、图标库配置为准。
- 按项目配置使用 shadcn CLI 添加或更新组件。组件源码与样式保持对应官方 registry 的默认实现；仅为修复明确的可访问性或功能缺陷修改组件源码，并记录原因。
- 组合页面时优先使用已有的 Button、Badge、Card、Dialog、DropdownMenu、Field、Input、InputGroup、NavigationMenu、Select、Table、Tabs、Textarea、Tooltip 等组件。交互状态使用组件自带的 `variant`、`size` 和 Radix 状态属性。
- 页面级 `className` 只负责布局：宽度、网格、排列、间距与响应式位置。不要用它改写组件的颜色、字号、圆角、阴影、高度、内边距或边框。

- 链接解析、本地视频与剧本文档使用共享大尺寸输入区与同排操作按钮。Input 使用 `controlSize="xl"`、Button 使用 `size="xl"`，高度 56px；保留官方颜色、圆角、焦点与错误状态，默认尺寸不变。
- 首页通过布局体现开阔感，不等比例放大文字和组件。标题与说明使用 PageHeader 的 lg 层级（手机 30px、桌面 36px，说明 16px），首页顶部复用 inner-page 的 24px / 32px 留白；三个功能入口采用方案 1 的左对齐紧凑 Tabs，不拉伸到整行，切换区右侧不放额外提示，输入与同排操作按钮沿用共享尺寸。通过标题、切换区、操作区的分组间距组织首屏，左右边界与导航栏一致。

## 色彩与字体

- 页面基底为白，文字为近黑；层次由 shadcn neutral 的 `background`、`foreground`、`card`、`muted`、`border` 等语义 token 表达。
- 深色模式为上述中性色的反转。主题只配置在 `frontend/src/app/globals.css` 的 light/dark token 中，不在业务组件里写一套手工 `dark:` 色彩覆盖。
- Logo 保留原有品牌色，不加黑白滤镜；页面基底和主要操作使用黑白/灰阶。不要使用彩色 mesh、彩色渐变、彩色装饰背景、霓虹光晕或彩色链接作为页面主视觉。
- 下载分析图表采用 shadcn 官方 Chart（Recharts）组合，以白色页面底面搭配深浅蓝色数据系列；深色模式沿用深色底面与对应蓝色 token。颜色统一配置为 `globals.css` 的 `chart-*` token，图例、提示和数据系列保持一致，并配合文字区分数据。错误状态可沿用 shadcn 官方 `destructive` 语义；状态不能只靠颜色传达。
- 正文和标题使用项目现有的 Geist 与中文系统回退字体。页面标题按内容层级排版，避免把普通页面标题做成营销海报字。

## 页面导航与页头

- 各业务页面的顶部返回区域统一复用 `PageNavigation`，使用官方 ghost Button 默认尺寸（32px 高、14px 文字），导航行高 32px，与后续内容间距 24px。页面不额外叠加导航外边距；加载、错误、详情状态使用同一结构。
- 当前业务页使用返回导航，不展示虚假的路径面包屑；保留站内历史返回和直接进入时的上级页面兜底。使用指南的路径导航使用官方 Breadcrumb，并放入同一高度的 PageNavigation 区域。
- 页头操作与普通表单操作使用 Button 默认尺寸，与默认输入控件对齐；首页 56px 大尺寸入口保留既定例外。
- 页头统一 PageHeader 的标题层级（手机 24px、桌面 30px）和 14px 说明；顶部站点导航维持 64px 高，各业务页面通过 inner-page 统一顶部留白。

## 布局与密度

- 页面和数据列表采用无边框设计，以留白、对齐和文字层级区分内容。表格不加外围描边、圆角框或行分隔线。
- 无边框表格是用户指定的共享样式例外：通过 `globals.css` 的 `table-borderless` 统一移除行边框，将正文单元格上下留白设为 12px；官方组件源码、水平间距、垂直居中、悬停、选中与键盘焦点状态保持不变。平台状态与共享 Data Table 使用同一规则，不为各页面复制样式。
- 平台状态采用一枚主要状态 Badge，接入信息使用次级文字，避免重复的描边标签堆叠。表单控件和弹层的必要边界不属于列表装饰边框。

- 同一页面的标题、表单、结果、列表和页脚共用内容容器左边界。响应式收窄时保持相同的阅读顺序。
- 留白应服务信息分组。不要用大面积空白、放大的 hero、高度固定的空区域或背景装饰补足页面。
- 页面与章节可使用简单容器、网格和 `gap-*`。不增加为单一页面定义的复杂视觉 CSS。
- 原生 shadcn 组件默认尺寸优先；相邻控件通过相同的 `size` 选择对齐，避免局部 `h-*`、`px-*` 让按钮和输入框变形。

## 组件与交互

- Button 使用官方 `variant` 与 `size`；不要改成自定义胶囊、阴影或品牌色。
- Tabs 使用官方 `Tabs`、`TabsList`、`TabsTrigger`、`TabsContent` 与内置 variant。保持 Radix 的键盘导航、焦点和选中态。
- Table 使用官方 `Table` 组合及上述共享无边框布局。列头与单元格保持相同列宽和文字对齐；数值列可右对齐，长文本可按列需求换行或截断。不要为各表复制一套不同的单元格 padding。
- Toast 使用 shadcn 的 Sonner 包装和 `sonner` 的 `toast()`；全站普通操作成功、失败及批量操作结果统一使用 Sonner toast，沿用根 Toaster 的位置与样式；同一解析任务从等待到失败更新同一条通知，不额外显示整行错误面板。字段校验、授权与需要持续操作的恢复状态放在对应页面内容中，危险确认使用 AlertDialog。
- 表单优先使用 Field、Input、InputGroup、Select 等现有组合，保留标签、错误说明、禁用状态和键盘焦点。
- 对话框、菜单、导航与提示使用 Radix/shadcn 原生语义和焦点管理。所有图标按钮须有可读名称。

## 列表与批量操作

- 分页统一右对齐，默认每页 10 条，可切换 10、20、50 条；切换大小或筛选条件回到第一页。
- 各数据列表不额外展示记录总数和顶部状态统计，分页保留每页条数、当前页与翻页按钮。未选择记录时不展示批量操作说明；选择后显示数量及适用操作。
- 选择框位于记录左侧并垂直居中，与内容和右侧操作分开；工具栏使用 FieldSet、Field、Checkbox 和同尺寸 Button。
- `disabled` 表示操作不可用，`busy` 只表示该操作的请求正在执行；无选择、列表刷新、其他操作执行中均不得让删除按钮显示 Spinner。
- 批量执行显示已处理数量与总数，并在结束后区分成功和失败；删除使用 AlertDialog 确认数量，失败项保留选择。

- 剧本文档、文件管理、用户、平台目录和 AI 配置列表统一提供本页多选与批量删除；分页或筛选变化清空选择。当前登录账户、当前 AI 线路及系统兜底线路不可批量删除。

## 验收

- 每次视觉变更先核对官方 CLI/文档与 `components.json`，预览组件差异。
- 在亮色与暗色、桌面与窄屏检查真实渲染；验证 Tabs 切换、表单提交、Toast、Table 对齐、键盘焦点与响应式布局。
- 提交前运行项目已有的格式、类型和相关测试。视觉改动以浏览器渲染为最终依据。
