...
6. Ensure Cursor review comments (if present) are acknowledged and any required
...
13. **Per-comment mode:** For each review comment, choose one of: accept,
    clarify, or push back. Reply inline (or in the issue thread for Cursor
    reviews) stating the mode before changing code.
14. **Reply before change:** Always respond with intended action before pushing
    code changes (inline for review comments, issue thread for Cursor reviews).
...
# Wait for review feedback: Cursor reviews arrive as issue comments that start
# with "## Cursor Review — <persona>". Treat them like reviewer feedback: reply
# with a `[cursor]` issue comment acknowledging the findings and whether you're
# addressing or deferring them.
while true; do
  gh api repos/{owner}/{repo}/issues/"$pr_number"/comments \
    --jq '.[] | select(.body | startswith("## Cursor Review")) | .id' | rg -q '.' \
    && break
  sleep 10
done
...
```
python3 .cursor/skills/land/land_watch.py
```
...
- Do not merge while review comments (human or Cursor review) are outstanding.
- Cursor review jobs retry on failure and are non-blocking; use the presence of
  `## Cursor Review — <persona>` issue comments (not job status) as the signal
...
- Cursor reviews now arrive as issue comments posted by GitHub Actions. They
  start with `## Cursor Review — <persona>` and include the reviewer’s
...
- Use review comment endpoints (not issue comments) to find inline feedback:
  ...
  - Reply to a specific review comment:
    ...
    gh api -X POST /repos/{owner}/{repo}/pulls/<pr_number>/comments \
      -f body='[cursor] <response>' -F in_reply_to=<comment_id>
    ```
...
- All GitHub comments generated by this agent must be prefixed with `[cursor]`.
- For Cursor review issue comments, reply in the issue thread (not a review
  thread) with `[cursor]` and state whether you will address the feedback now or
  defer it (include rationale).
- If feedback requires changes:
  - For inline review comments (human), reply with intended fixes
    (`[cursor] ...`) **as an inline reply to the original review comment** using
    the review comment endpoint and `in_reply_to` (do not use issue comments for
    this).
  - Implement fixes, commit, push.
  - Reply with the fix details and commit sha (`[cursor] ...`) in the same place
    you acknowledged the feedback (issue comment for Cursor reviews, inline reply
    for review comments).
  - The land watcher treats Cursor review issue comments as unresolved until a
    newer `[cursor]` issue comment is posted acknowledging the findings.
- Only request a new Cursor review when you need a rerun (e.g., after new
  commits). Do not request one without changes since the last review.
  - Before requesting a new Cursor review, re-run the land watcher and ensure
    there are zero outstanding review comments (all have `[cursor]` inline
    replies).
  - After pushing new commits, the Cursor review workflow will rerun on PR
    ...
    ```
    [cursor] Changes since last review:
  ...
  - Only request a new review if there is at least one new commit since the
    previous request.
  - Wait for the next Cursor review comment before merging.
...
- If review feedback expands scope, decide whether to include it now or defer
  ...
  call it out in the root-level `[cursor]` update with a brief reason (e.g.,
