...
def latest_review_request_at(comments: list[dict[str, Any]]) -> datetime | None:
    ...
    for comment in comments:
        if is_cursor_bot_user(comment.get("user", {})):
            continue
        body = comment.get("body") or ""
        if "@cursor review" not in body:
            continue
...
def filter_cursor_comments(
    comments: list[dict[str, Any]],
    review_requested_at: datetime | None,
) -> list[dict[str, Any]]:
    latest_cursor_reply = latest_cursor_reply_by_thread(comments)
    latest_issue_ack = latest_cursor_issue_reply_time(comments)
    cursor_comments = [c for c in comments if is_cursor_bot_user(c.get("user", {}))]
    ...
    for comment in cursor_comments:
        ...
        if not is_threaded:
            ...
            if thread_root is not None:
                last_reply = latest_cursor_reply.get(thread_root)
...
def is_cursor_bot_user(user: dict[str, Any]) -> bool:
    login = user.get("login") or ""
    return login in CURSOR_BOTS


def is_bot_user(user: dict[str, Any]) -> bool:
    login = user.get("login") or ""
    if is_cursor_bot_user(user):
        return True
...
def is_cursor_reply_body(body: str) -> bool:
    return body.startswith("[cursor]")


def is_cursor_review_body(body: str) -> bool:
    return body.startswith("## Cursor Review")


def latest_cursor_issue_reply_time(
    comments: list[dict[str, Any]],
) -> datetime | None:
    latest: datetime | None = None
    for comment in comments:
        body = (comment.get("body") or "").strip()
        if not is_cursor_reply_body(body):
            continue
        created_time = comment_time(comment)
        if created_time is None:
            continue
...
def filter_human_issue_comments(comments: list[dict[str, Any]]) -> list[dict[str, Any]]:
    latest_ack = latest_cursor_issue_reply_time(comments)
    filtered: list[dict[str, Any]] = []
    for comment in comments:
        if is_bot_user(comment.get("user", {})):
            continue
        body = (comment.get("body") or "").strip()
        if is_cursor_reply_body(body):
            continue
        if is_cursor_review_body(body):
            continue
        if "@cursor review" in body:
            continue
        created_time = comment_time(comment)
        if (
...
def filter_cursor_review_issue_comments(
    comments: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    latest_ack = latest_cursor_issue_reply_time(comments)
    filtered: list[dict[str, Any]] = []
    for comment in comments:
        body = (comment.get("body") or "").strip()
        if not is_cursor_review_body(body):
            continue
        created_time = comment_time(comment)
        if (
            latest_ack is not None
...
def latest_cursor_reply_by_thread(
    comments: list[dict[str, Any]],
) -> dict[int, datetime]:
...
def filter_human_review_comments(
    ...
    for comment in comments:
        ...
        if thread_root is not None:
            last_cursor_reply = latest_cursor_reply.get(thread_root)
        if last_cursor_reply and created_time and created_time <= last_cursor_reply:
            continue
        filtered.append(comment)
    return filtered
...
def is_blocking_review(
    ...
    if user_login in CURSOR_BOTS:
        return state == "CHANGES_REQUESTED"
    if body.startswith("[cursor]") or state in ("APPROVED", "DISMISSED"):
        return False
    blocking = False
...
def raise_on_human_feedback(
    ...
    cursor_review_comments = filter_cursor_review_issue_comments(issue_comments)
    human_review_comments = filter_human_review_comments(review_comments)
    if human_issue_comments or human_review_comments or cursor_review_comments:
...
async def wait_for_cursor(pr_number: int, checks_done: asyncio.Event) -> None:
    ...
    while True:
        ...
        bot_issue_comments = filter_cursor_comments(issue_comments, review_request_at)
        bot_review_comments = filter_cursor_comments(review_comments, review_request_at)
        ...
        if bot_comments:
            ...
            if body:
                print("Cursor left comments. Address feedback before merge.")
                print(body)
                raise SystemExit(2)
...
async def watch_pr() -> None:
    ...
    cursor_task = asyncio.create_task(wait_for_cursor(pr.number, checks_done))
    ...
    success_task = asyncio.gather(cursor_task, checks_task)
