...
请只处理你拥有相应权利的内容。匿名 Provider 默认只处理能够正向证明为公开、免费、非 DRM 的媒体；Edge Agent 只能传输用户已经合法取得并显式选择的明文文件，不能读取平台会话、拦截流量或转换受保护内容。受控媒体只有在官方资产连接器明确返回下载/导出授权且输出未加密时才可生成 Artifact；会员、购买、私密、地域或 DRM 播放权益不等于第三方明文导出权。
