...
根目录两份 Compose 各自承担固定职责，不使用 `deploy/` 目录：
...
本地默认配置已经写在 Compose 中；只有需要覆盖密码、对象存储公开地址或 AI Provider
时，才复制 `.env.example` 为被 Git 忽略的 `.env`。
...
服务入口默认为 <http://localhost:8101>。仅启动基础设施使用 `docker-compose-env.yml`；完整系统使用 `docker-compose.yml`。容器内连接始终使用 Compose DNS，不会被 `.env` 中宿主机连接地址覆盖。
