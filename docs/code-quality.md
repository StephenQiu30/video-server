...
证据基线与完整方案见 [021 深度调研](research/021-下载可靠性与重启会话故障深度调研.md)。优先级沿用本登记定义；共同出口故障为本轮最先处置项。以下均未关闭，本次未部署业务修复。

- **CQ-029 / P1 / 行为与验证缺陷 / 已修复待生产验收**：Squid 前台 PID 1 将 PID 文件写入无 tmpfs 的容器 `/tmp`，异常停止后残留文件导致重启持续退出。实际日志、795 次重启快照及同镜像隔离 SIGKILL/start 已复现。两个业务 Compose 已为代理 `/tmp` 增加 tmpfs，保留 PID 健康检查和现有 ACL；新增契约测试先失败后通过。使用当前代理镜像和完整生产 ACL 的候选容器完成正常停止/start 3 轮及 SIGKILL/start 3 轮，六轮均 ready 且 `squid -k check` 通过；临时容器已清理。生产替换、实际转发、Runner 恢复和原路线真实文件仍待验收。不得以候选通过或删除一次 PID 关闭。
