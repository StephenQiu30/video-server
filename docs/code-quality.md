...
- **CQ-034 / P1 / 来源所有权与诊断缺口 / S0 已修复，来源独立未完成**：平台来源仍依赖日常 Chrome 数据库、宿主隐私授权与仓库虚拟环境；快速 probe 不证明来源可读取。`31f0811e` 将 Profile/数据库缺失、OS 权限拒绝、确实缺少必需 Cookie 和一般来源故障分开，新增并行有界且不输出 Cookie 的 `doctor`；默认 operator 策略缺 endpoint 以及 operator Canary 目标缺 endpoint 均在启动前拒绝。`cd1570b4` 使 Canary 将来源故障保留为 `provider_configuration_missing`。最终 121 项聚焦回归通过；此前同一实现的后端全量 1783 passed / 2 skipped，最终映射变更另由聚焦用例覆盖；Ruff/格式及 mypy 534 个源文件通过，两套 Compose 配置解析通过。跳过项是未提供隔离 MinIO 和 macOS 不具备 Linux `O_PATH`，与本修复无新增失败。本机脱敏诊断实测：抖音、Facebook、Instagram、Pinterest、Reddit、X、小红书、YouTube 均为 `provider_session_permission_denied`，专用元宝视频号为 `ok`；因此不再把这八项提示为需要重新登录。生产使用两次同源镜像增量替换受影响服务，YouTube operator metadata 记录为 `provider_configuration_missing`；精确 TCC 责任进程仍未证实，S1/S3 项目独立来源、T23–T27 和真实完整文件继续未完成；动态修订关联 CQ-020，不重复实现租约/任务框架。
...
- **CQ-036 / P1 / 多平台验证路线缺陷 / 候选已修复**：固定公开矩阵仍从“平台存在浏览器会话策略”推导 X、Instagram、Facebook、Pinterest 必须使用 operator，导致四个平台受日常 Chrome 权限影响且生产周期监控缺席。同链接生产匿名对照的 metadata/media 均成功：X 12837/27449 ms、Instagram 7861/16667 ms、Facebook 5461/9870 ms、Pinterest 2473/25288 ms；Reddit 匿名两阶段均为 `provider_auth_required`，YouTube、抖音、视频号 metadata 分别为 `provider_verification_failed`、`provider_temporarily_unavailable`、`provider_auth_required`。候选将成功的四平台固定路线改为 anonymous，并把全部九个会话能力平台分入“已证明匿名”或“当前保留 operator”两个显式集合；以后新增平台必须明确归组。后端全量 1785 passed / 2 skipped，Ruff/格式及 mypy 534 个源文件通过；跳过项仍为隔离 MinIO 未提供和 macOS 不具备 Linux `O_PATH`。部署、生产定时目标更新及重建复测完成前不关闭。
