...
本轮依据基线 `f547b9e4` 与 [023 调研](research/023-内容创作与平台发布能力调研.md)。以下分清现有行为缺口、新需求和外部接入风险；未做实施/真实 UI 验收，不关闭任何项目。036 也不关闭 035 的来源/换机问题。

- **CQ-037 / P1 / 证据回看行为缺口 / 静态确认，待修复**：`analysis-panel.tsx` 两处 `AnalysisResultView` 未传 `onSelectTime`；文章结果页的 evidence 时间码也是普通文本。用户有时间证据却不能直接通过这些结果定位播放器。最小修复是传递现有播放器回调并支持文章证据，媒体已清理时给出准确状态；通过 036 A04 浏览器真实定位后关闭。
