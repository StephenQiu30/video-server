...
- **CQ-043 / P1 / 已登录 Chrome 与采集进程授权错位 / 来源安装已实现，真实交付待验收**：当前来源 open=EPERM，TCC 归属 Visual Studio Code；系统设置 ChatGPT FDA 已开、VS Code 关闭。新增部署侧显式 capture-chrome/import/check，按平台采集并原子发布只读来源，后续操作复用既有 file keyed revision，不再要求用户登录测试浏览器。56 项聚焦回归通过；完整文件、生产来源切换及重启后下载尚未完成，不能因 doctor 分类正确或文件格式有效关闭 CQ-034/G1。详见 035 当日验收。
