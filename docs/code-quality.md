...
- **CQ-044 / P1 / YouTube 静态会话轮换 / 未修复**：同一已验证来源完成候选重启后，生产仍明确返回 account cookies are no longer valid；本地 ProviderCookieFile 校验通过，故不能通过重启或格式检查认定账号有效。再次显式采集同一 Chrome 后正常网页完整任务成功，证明静态文件不是持续性方案。按 035 S3 实施单平台来源维护：平台接受验证后原子发布版本，失败保留来源、冷却与准确过期状态；下载冻结已有 revision，不在每次点击中读取浏览器或自动改用账号。需要跨轮换持续验收，不能把定时重新导出或单次成功当作永久登录，也不要求用户登录测试浏览器。
