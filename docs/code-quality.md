...
- CQ-025 / P1 / 待来源方案批准与实现：元宝原 Chrome 页面已登录，但按域 Cookie 查询为空，临时 Profile 只复制 Cookie，无法恢复该登录。空来源现已快速返回且不启动浏览器；需要准确获取当前元宝网页存储或由用户批准专用浏览器登录来源；不能全量复制个人 Profile、扫描恢复旧凭据或谎报为用户未授权。视频号完整下载仍失败，不能关闭。
