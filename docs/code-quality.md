...
- **CQ-034 / P1 / 来源所有权与诊断缺口 / S0 已修复，来源独立未完成**：平台来源仍依赖日常 Chrome 数据库、宿主隐私授权与仓库虚拟环境；快速 probe 不证明来源可读取。S0 已将 Profile/数据库缺失、OS 权限拒绝、确实缺少必需 Cookie 和一般来源故障分开，新增并行有界且不输出 Cookie 的 `doctor`；默认 operator 策略缺 endpoint 以及 operator Canary 目标缺 endpoint 均在启动前拒绝。105 项聚焦回归通过；后端全量 1783 passed / 2 skipped，Ruff/格式及 mypy 534 个源文件通过，两套 Compose 配置解析通过。跳过项是未提供隔离 MinIO 和 macOS 不具备 Linux `O_PATH`，与本修复无新增失败。本机脱敏诊断实测：抖音、Facebook、Instagram、Pinterest、Reddit、X、小红书、YouTube 均为 `provider_session_permission_denied`，专用元宝视频号为 `ok`；因此不再把这八项提示为需要重新登录。精确 TCC 责任进程仍未证实，S1/S3 项目独立来源、T23–T27 和真实完整文件继续未完成；动态修订关联 CQ-020，不重复实现租约/任务框架。
