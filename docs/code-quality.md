...
| CQ-013 | P3 / 失效文件与引用 | 闲置组件、旧预览数据和已删除清单的构建引用 | 已清理；300 项前端、6 项后端测试与镜像构建通过 | 文件清理 |
...
### CQ-013：闲置前端文件与失效构建入口

证据：a337e903 之后，Checkbox 组件无源码或测试导入；demo-inspection 和山景图片只服务于旧开发预览入口。供应链清单已从工作区删除，但 Docker COPY 与清单快照测试仍引用它们；视觉规范文档只转发根 design.md。
