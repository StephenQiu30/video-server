...
- **CQ-029 / P1 / 行为与验证缺陷 / 方案就绪**：Squid 前台 PID 1 将 PID 文件写入无 tmpfs 的容器 `/tmp`，异常停止后残留文件导致重启持续退出。实际日志、795 次重启快照及同镜像隔离 SIGKILL/start 已复现。最小修复是两个业务 Compose 为代理 `/tmp` 增加 tmpfs，保留 PID 健康检查和现有 ACL；隔离候选通过，完整生产配置、转发、正常/强杀多轮及原路线真实文件待验收。不得以重建一次或删 PID 临时恢复关闭。
