...
- **CQ-034 / P1 / 来源所有权与诊断缺口 / S0 已修复，S1 部分通过，持续维护和换机未完成**：`31f0811e`/`cd1570b4` 已区分来源缺失、权限拒绝与账号缺失并完成 endpoint 预检，原 121 项聚焦回归通过。后续 TCC 已证实本次采集进程归属 VS Code，开启实际执行宿主授权后既有 Chrome 来源可读，详见 CQ-043。YouTube/抖音/Reddit 生产已改为单平台只读文件，候选完整媒体和重启通过，YouTube 正常网页任务已持久化并全片解码。文件读取不依赖日常 Chrome，但维护仍依赖显式采集；YouTube 上游轮换已复现并登记 CQ-044，不能关闭 S3。真正第二机器、三次干净恢复、跨轮换与撤销等 T23–T27 范围尚未全部验收；动态身份修订继续关联 CQ-020。
...
- **CQ-044 / P1 / YouTube 静态会话轮换 / 未修复**：同一已验证来源完成候选重启后，生产曾明确返回 account cookies are no longer valid；再次显式采集同一 Chrome 后正常网页完整任务和重建复测成功。后续 agent-browser 路线复核期间，同一文件又先返回 `inspection_failed` 9488 ms，再返回 `provider_session_expired` 5410 ms。本地 ProviderCookieFile 校验通过，证明静态快照不是持续性方案。按 035 S3 实施独立单平台维护者：在用户请求之外验证并原子发布新 revision，失败保留来源、冷却与准确过期状态；Runner 仍只读精确版本。不得以反复手工 capture-chrome、增加账号或每次点击读取浏览器伪装稳定，也不要求用户登录测试浏览器。
