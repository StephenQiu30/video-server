...
- [下载故障修复与持续可用性 SOP](operations/009-下载故障修复与持续可用性SOP.md)：按当前仓库规范执行接报、分层诊断、四份文档对齐、TDD、候选验证、部署、重启/文件验收、回滚和缺陷关闭；当前流程已制定，修复尚未执行。

- [下载可靠性与重启会话故障深度调研](research/021-下载可靠性与重启会话故障深度调研.md)：2026-09-15 实测定位 Squid 残留 PID 重启循环，复现 Fresh cookies 登录误分类；包含 23 平台维护策略、修复切片与验收矩阵。研究完成，生产修复与全平台实测尚未完成。
