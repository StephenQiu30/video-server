...
- Pinterest、微博、优酷保留当前版本化匿名公开单视频 Profile；受保护或多资产内容继续 fail closed。
...
- 只有管理员可以新增、查询、修改或删除平台目录条目；普通用户和未登录请求必须由后端拒绝。
- 目录只允许维护稳定 key、显示名称、排序值和公开可见性，并驱动 `GET /api/providers` 的展示顺序与范围。
- 目录维护不得创建域名规则、Extractor、Runner 参数、会话权限或下载能力；没有后端安全 Profile 的自定义条目必须显示为 `unsupported`。
- 删除采用可恢复的逻辑删除，数据库初始化不得把管理员已删除的默认条目重新插入。
