...
- 状态：Accepted
- 关联 Design：`docs/design/archive/001-server单仓与运行时架构设计.md`
...
1. 仓库必须包含 `backend/`、`frontend/`、`docs/` 三个明确模块，Dockerfile 和两种 Compose 配置直接位于根目录。
2. 前后端可独立安装依赖、测试和开发；生产构建必须把前端静态产物放进统一 Server 镜像。
3. API、下载 Worker、媒体 Runner 和 AI Worker必须有独立进程入口，可分别扩缩容和重启。
4. 根目录必须提供统一环境模板、职责分层的两份 Compose（本地完整配置、生产环境覆盖）、CI 与操作说明。
