...
| 部署方式 | Docker Compose | 后端部署统一管理 API、Worker、Redis、PostgreSQL、MinIO；前端在 `video-web` 独立仓库用 Docker / Nginx 部署静态资源。 |
| 项目结构 | 双仓库拆分 | `video-server` 承载 API/Worker/shared；`video-web` 承载 React 前端。 |
| 变更管理 | `bootstrap-mvp-foundation` | M1 实现变更使用 变更管理 proposal、design、specs、tasks。 |
...
- 变更管理：M1 变更名 `bootstrap-mvp-foundation`。
- 部署：Docker Compose。
...
- [x] M1 变更管理：变更管理 `bootstrap-mvp-foundation`。
