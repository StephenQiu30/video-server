...
| UI 体系 | Ant Design Pro | 使用 Ant Design Pro / Umi 体系，优先复用 ProLayout、ProTable、ProForm、登录页模式。 |
...
| 文件存储 | MinIO / S3 兼容对象存储 | 使用私有 bucket + 后端生成短期预签名 URL 的成熟模式。 |
| AI 摘要 | 二期再做 | 首版不实现 AI 摘要、问答和思维导图。 |
| 鉴权 | JWT 用户系统，允许注册 | 首版提供注册、登录、当前用户和任务归属。 |
...
- 存储：MinIO / S3 兼容对象存储。
- 鉴权：JWT 用户系统，允许注册。
- 资源限制：单任务最大 2GB、最长 2 小时、全局并发 2、单用户并发 1、文件保留 24 小时、预签名 URL TTL 15 分钟。
...
- [x] 任务队列：RQ + Redis。
- [x] 数据库：PostgreSQL。
- [x] 文件存储：MinIO / S3 兼容对象存储，私有 bucket + 预签名 URL。
- [x] AI 摘要：二期再做。
- [x] 账号系统：JWT 用户系统，允许注册。
