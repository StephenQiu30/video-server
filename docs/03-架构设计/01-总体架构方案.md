...
采用“React + Umi + Ant Design Pro 前端 + FastAPI 后端 + RQ 后台任务 + 下载内核可插拔”的架构。首版优先保证本地 Docker Compose 可运行、任务可追踪、错误可解释，并以 PostgreSQL、Redis、MinIO / S3 兼容对象存储和允许注册的 JWT 用户系统作为基础能力。
...
| 模块 | 职责 |
| --- | --- |
| Web 前端 | React + Umi + TypeScript + Ant Design Pro，负责注册登录、链接输入、格式选择、任务台、历史记录、错误反馈 |
| 后端 API | FastAPI，负责参数校验、JWT 注册登录、任务创建、状态查询、文件授权、错误转换 |
