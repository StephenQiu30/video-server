...
- 关联设计：`docs/design/010-Codex与Claude CLI视频分析设计.md`、`docs/design/archive/012-AI分析报告与MinIO持久化设计.md`、`docs/design/archive/014-WebSocket任务状态同步设计.md`、`docs/design/archive/015-RabbitMQ异步分析设计.md`
...
设计冻结时，`analysis_jobs.attempt` 已支持 Worker 对限流、超时、无效模型输出等可重试故障进行自动重投，并保持同一个 job id。缺口在手动操作：前端“重新分析”会清空当前状态，再次调用 `POST /api/downloads/{download_id}/analyses`，从而创建新的 `analysis_jobs` 记录。
