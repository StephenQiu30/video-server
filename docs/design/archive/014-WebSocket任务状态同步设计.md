...
- 关联设计：`docs/design/archive/013-AI分析原任务重试设计.md`、`docs/design/archive/015-RabbitMQ异步分析设计.md`、`docs/design/006-上线产品能力补全设计.md`
...
设计冻结时，分析前端在非终态下按固定间隔调用 `GET /api/analyses/{id}`，页面卸载或进入终态后停止。该方式在任务多、运行时间长时产生重复查询，也不能自然承载 013 的执行代次和 012 的报告发布事件。
