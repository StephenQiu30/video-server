...
服务端把同一份结构化证据编排为公众号式分析文章：先说结论，再依次呈现内容推进、逐分镜证据、关键亮点、优先建议、视觉资产和分析口径。镜头景别、运动和转场在展示层本地化为自然语言；缺少可选内容时使用克制的编辑式说明，不为凑章节生成虚构判断。时间码、分镜编号和证据引用继续保留，保证文章感不以牺牲可核验性为代价。
...
视觉分析的预览、Markdown 下载和 DOCX 下载继续消费唯一规范 Markdown。DOCX 只增加编辑式页眉、主题和页脚，不维护第二套报告文案，因此网页与导出件的标题、章节顺序、表格证据和安全转义保持一致。
