...
首个实现采用 OpenAI Python SDK 的当前 Responses API 与 Audio Transcriptions API，分别配置 ASR 与文本模型；`base_url` 只允许由受信部署配置覆盖。密钥只注入 `worker-analysis`，API 和前端不得读取 Provider 配置。
...
- `POST /api/v1/downloads/{download_id}/analyses`
