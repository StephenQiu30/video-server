...
在用户本轮授权下，macOS 来源覆盖增加抖音和 Reddit，均复用现有单平台加密队列，不添加协议兼容或备用账号。视频号沿用元宝来源；实际 Chrome 已登录却无该域 Cookie，证明仅复制 Cookie 的临时 Profile 不能恢复当前登录。不得把缺失的网页存储状态归咎于用户未授权，也不得全量复制 Chrome Profile 或恢复历史已删除凭据。网页存储来源未验证前，视频号保持未通过。

宿主代理保持一个按需启动进程，按 Provider 有界并行导出，同平台最多一个导出；主线程持续响应无凭据 probe，已空闲的平台可以处理后来请求，避免元宝的慢操作阻塞其他平台。所有子导出保留 15 秒预算和进程组清理；临时 Chrome 不再另建进程组，以便外层取消覆盖整个树。代理空闲即退出，不引入常驻维护服务或新队列。
