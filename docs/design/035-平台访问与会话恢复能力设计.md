...
用户明确允许当前部署按需使用本机 Chrome 的 YouTube 会话。复用现有按域查询、加密单次租约、独立 Runner 和 tmpfs；不复制 Profile、不保存明文来源、不实现登录/验证码或新浏览器服务。可选 `docker-compose-browser.yml` 仅替换生产 YouTube 的文件挂载为既有代理的 YouTube 队列，其他平台及 Linux 文件部署不变；默认策略在部署环境中显式设为 `operator_public`。设置 `COMPOSE_FILE` 保留重建时的同一拓扑。首次隔离实验取得原视频公开、564 秒、360p 音视频 MP4 格式；须继续验证完整文件和重启，不将 metadata 当交付成功。已有动态来源固定 browser 版本不证明账号切换检测，账号变更/新机仍需独立验收。
...
macOS 来源覆盖包含抖音、Reddit 和视频号，均复用现有单平台加密队列，不添加协议兼容或备用账号。普通 Chrome 的元宝登录不能仅靠 Cookie 恢复，因此视频号只消费上述独立持久来源，不全量复制 Chrome Profile 或恢复历史已删除凭据。来源导出成功与真实媒体下载分别验收，不得把技术来源缺陷归咎于用户未授权。
