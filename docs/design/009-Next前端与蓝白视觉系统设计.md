...
颜色必须通过语义 token 使用。主按钮使用 Apple 蓝与白字，不使用纯黑主按钮、大面积黑底或装饰性渐变。Apple 蓝只用于主操作、链接、焦点、选中/当前步骤和确有语义的状态，不得用作装饰性信息、页面分类或标题层级标记。状态不能只靠颜色表达，必须同时包含文字或图标。

### 字体、间距与容器

- 字体栈以 Next.js 自托管 Geist Sans 为西文与数字基线，中文依次回退到 `PingFang SC`、`Hiragino Sans GB`、`Microsoft YaHei` 与系统无衬线字体；运行时不从第三方加载字体。尺度参考 [Geist Typography](https://vercel.com/geist/typography)：首页 H1 在最宽断点上限为 56px/1.08；除首页 Hero 外，所有内页 H1 固定为桌面 32px/1.2、390px 28px/1.2；正文为 14‑16px/1.5，标签和元数据为 12‑14px/1.4。小尺寸不得靠过度字重补偿可读性。
- 所有路由的页面级 Header、main 与 footer 复用同一最大 1200px PageShell：桌面宽度为 `min(calc(100% - 48px), 1200px)`，390px 视口宽度为 `min(calc(100% - 32px), 1200px)`。路由组件不得另设 1440px 或其他页面级最大宽度。认证表单、资料表单、说明文字和媒体等可在 PageShell 内二次收窄，但不得收窄页面根容器、改变页面标题对齐线或让 Header 与正文使用不同边界。内容区以 12 列理解对齐：Hero 和输入可跨全宽，结果区在桌面端以 5/7 或 6/6 列分配，不用互不对齐的任意宽度。
- 间距采用 4px 基准，控件内部使用 8/12/16px，内容组使用 24/32px，主要段落使用 48/64/96px。标题、描述和主操作之间必须保留明显层级，不通过额外卡片填满空白。
- 除首页 Hero 外，所有内页 PageHeader 统一由 H1、一句可选的 muted 说明和可选主操作组成；页面或区块标题上方不得出现装饰性 eyebrow/kicker、全大写英文标签、等宽体分类词，或与 H1 重复的“任务记录”“账户设置”“系统管理”等说明。此类重复眉题直接删除，不改成灰色文案替代。
- Header 固定视觉高度为 64px，与主网格共用水平安全区，下沿只使用 1px 发丝线。不使用大面积模糊、浮起阴影或黑色顶栏。
- 页面根、PageHeader、筛选区、列表区和表单区默认不使用可见 Card 外框、ring、阴影或装饰性大边框。输入框、选择器、表格/列表分隔、预览图、焦点、错误、Dialog 和 Sheet 等需要辨识或焦点圈定的元素可使用 1px 发丝线和统一的 10px 圆角。需要复用官方 Card 语义时使用 `border-0`、`ring-0`、`shadow-none`，不得 Card 套 Card。
...
| 信息/交互语义 | 官方组件 | 项目使用规则 |
| --- | --- | --- |
| 独立内容分组 | [Card](https://ui.shadcn.com/docs/components/radix/card) | 只用于有独立标题/操作的单个语义容器；默认无可见 border/ring/shadow，登录/注册最多使用一层 Card，不嵌套 |
...
桌面端为 64px 单行轻量 Header：左侧品牌标识“帧取”保留指向 `/` 的辅助品牌链接；右侧主导航依次提供“视频解析”（`href="/"`）、“下载历史”（`href="/history"`）和 Avatar 账户菜单，管理员入口位于账户菜单。品牌链接不能替代“视频解析”这一显式导航项。Header 与页面使用同一 1200px 网格，底部仅有 1px 发丝线，没有侧栏、面包屑容器、浮起阴影或固定黑色顶栏。主导航根据 pathname 只为当前路由链接设置 `aria-current="page"`，并通过浅色状态面和文本色同步表达当前页，不使用下划线。

390px 下保留品牌和一个明确的导航触发器，“视频解析”与“下载历史”两个带文字的路由链接以及账户操作进入 Sheet；不把桌面导航强行压缩到同一行。Sheet 打开后焦点进入其可操作内容，两个路由链接均可通过 Tab 到达并以键盘激活；从 `/history` 激活“视频解析”后导航到 `/` 并关闭 Sheet。当前链接在 Sheet 内同样使用 `aria-current="page"`，未选中链接不携带该属性。关闭而未导航时，焦点返回导航触发器。图标按钮有可见或屏幕阅读器标签，Tooltip 仅作辅助，触控区域至少 44×44px。

桌面端从 `/history` 回到解析页的最短键盘路径为：进入 Header 主导航、聚焦“视频解析”、按 Enter 激活并到达 `/`；移动端路径为：聚焦导航触发器并打开 Sheet、聚焦“视频解析”、按 Enter 激活并到达 `/`。品牌链接提供冗余回路，但不作为这两条验收路径的前提。

### 首页 `/`

首页严格延续方案 2：

1. 首屏中心直接使用“粘贴链接，剩下的交给帧取”和一行支持平台说明，文案短而直接；H1 上方不添加 `PUBLIC MEDIA WORKFLOW` 等装饰性英文眉题或蓝色分类词。
...
4. 解析成功后展示两栏结果：左侧为语义格式单选列表和全宽“开始下载”，右侧为真实封面、标题、平台与时长。格式行使用 Item/单选语义、发丝分隔和淡蓝选中面，媒体用 AspectRatio 固定 16:9；整个结果最多使用一层无可见外框的 Card 语义容器，不包卡片套卡片。
...
PageHeader 直接显示“下载历史”及一句用途说明、可选的“新建下载”主操作，不在 H1 上方显示“任务记录”等重复眉题。搜索、状态筛选和刷新紧随标题区。桌面端与移动端均使用 Item/ItemGroup，行间只有 ItemSeparator；视频标题和封面构成第一信息层级。桌面端展开为宽行并保留更完整元数据，390px 下重排为单列，每项展示标题、格式、状态、时间和唯一主操作，不通过横向滚动隐藏内容。Pagination 在桌面端可展示页码，390px 下精简为上/下页和当前页说明。

空状态、筛选无结果、加载、刷新失败和分页均有明确文案；无数据使用 Empty 组合，不用一个空 Card 充位。成功任务提供“获取文件”，其他任务提供“查看任务”。无论列表处于加载、空、错误或正常状态，全局导航中的“视频解析”都保持可用，使用户无需依赖浏览器后退即可开始新的解析。
...
在统一 PageShell 内使用居中、窄列的单层官方 Card 表单，Card 不嵌套其他 Card且不显示装饰性 border/ring/shadow。登录突出邮箱、密码和单一蓝色主按钮；注册增加用户名与密码确认。标题直接显示“登录”或“创建账户”，不增加“欢迎回来”“开始使用”等彩色眉题。字段统一由 Field + InputGroup 组合 label、描述、密码可见性与错误，错误出现在对应字段附近并在提交失败时聚焦摘要。登录态用户访问认证页时返回安全的站内目标。
