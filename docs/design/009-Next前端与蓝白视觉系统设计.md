...
空状态、筛选无结果、加载、刷新失败和分页均有明确文案；无数据使用 Empty 组合，不用一个空 Card 充位，也不默认添加装饰性圆角图标底座。成功任务提供“获取文件”，其他任务提供“查看任务”。初始加载骨架必须为最终统计摘要保留相同的 18px 高度和上下间距，数据到达时不得把列表区域向下推移。无论列表处于加载、空、错误或正常状态，Header 中的品牌首页链接都保持可用，使用户无需依赖浏览器后退即可开始新的解析。
