...
状态：个人处理记录已实现；2026-09-25 增加管理员日志架构，实现与验收证据见 Plan。关联：[PRD](../prd/045-统一解析中心PRD.md) · [Plan](../plan/045-统一解析中心Plan.md)。
...
个人处理记录是既有领域资源的统一只读查询。保留现有 HistoryRecordRepository → Service → Schema → 路由，不新增万能 Task 表、History 写入表或异步投影消费者。数据库现有记录立即纳入查询，不需要复制状态、补写任务或改动 outbox。
...
Web 用页码控件呈现游标分页：URL 中每个游标对应下一页起点，当前页为游标数加一；可直接返回已走过的页，下一页只在响应给出 next_cursor 时可达。显示“第 N 页”和“上一页/下一页”，不展示“更早的记录”，也不把已知的下一页误称为总页数。筛选改变或显式刷新回到第一页，刷新时只发起该页的一次读取。运行记录采用相同页码控件。

新插入记录位于首部，旧页不会自动混入；首部显示有新记录提示，显式刷新重置游标。状态筛选下，任务变化可以退出当前集合，这是活跃数据语义；不承诺跨多次请求冻结快照。删除/状态变化导致当前页为空时可返回上一页。无需 COUNT(*) 或为页签默认查询总数。
...
- 创建/取消/重试/删除成功后失效所有当前身份的我的处理记录筛选缓存与来源详情；保留滚动和筛选。不因为写操作完成在已经离开的页面强制导航。
...
系统日志与个人当前态聚合查询分开。`operation_logs` 记录请求生命周期和系统任务状态事件，不替代领域任务表。`OperationLogRoute` 捕获已匹配的写请求，路由执行前提交 started 记录；认证依赖写入请求上下文身份，完成后仅允许一次结果写入。所有管理员配置和权限变更共享该边界。原生接口沿用自身响应协议，仅复用日志路由。

SQL 触发器覆盖 download_intents、download_jobs、analysis_jobs、documents、media_imports 的新建、状态变化和删除，在业务事务中写入安全状态事件。日志不持有用户或资源外键；不记录完整 payload，不补写历史状态。

管理员查询由独立权限依赖保护，返回类型从 OpenAPI 生成。UI 使用已有 shadcn Table、Field、Select、Dialog 与统一 PagePagination，入口 `/admin/operation-logs`。个人页面迁至 `/history/activity`，保持已有 owner 查询和业务动作。具体记录字段及运行故障处理见[运行说明](../operations/system-operation-logs.md)。
