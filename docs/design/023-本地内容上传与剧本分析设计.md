...
`source_scene_id` 仅供服务端核对场景覆盖与引用归属。它由上传剧本的场景标题生成，不是截图、原文摘录或已经核实的事实依据。网页和规范报告不展示原始 ID，也不把 ID 列表标为“证据”；逐场景章节按原文顺序编号，并提醒读者对照上传的原文核查 AI 判断。历史报告版本保持不可变，新规则从下一次执行生成的新版本生效。
