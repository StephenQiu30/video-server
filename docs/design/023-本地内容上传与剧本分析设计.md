...
`source_scene_id` 仅供服务端核对逐场景覆盖。它由上传剧本的场景标题生成，不是截图、原文摘录或已核实的事实依据。分析契约不含场景“证据”列表；网页和报告按原文顺序给逐场景章节编号，并提醒读者对照上传的原文核查 AI 判断。切换当前契约时一次性整理现存剧本分析结果和报告，不保留旧格式读取分支；之后的新报告版本保持不可变。
...
成功的 `screenplay-analysis` 以结构、人物、场景、对白、修改建议和完整报告六个键盘 Tabs 组织；不展示模型生成的场景引用编号。`screenplay-rewrite` 以源/输出场景统计、语义表格 glossary、修改摘要和 canonical Markdown 改写正文组织，不从公开 result 拼装第二份全文。两类结果只在报告同时具备 Markdown/DOCX Artifact 时显示导出动作。
