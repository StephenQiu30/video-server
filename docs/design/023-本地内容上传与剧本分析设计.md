...
当前已落地的单次调用路径权限更窄：只支持 Claude CLI 的 `--tools ""` 单轮结构化调用，规范化剧本以 JSON 字符串嵌入标准输入，CLI 不获得 Read、Write、Edit、Bash、网络、FFmpeg、MCP、插件、浏览器、Agent 或 subagent。工作区中的 `input/screenplay.md` 仅供父 Worker 完整性复核，`work/chunks` 和 `work/outputs` 为后续可信分块编排保留，当前模型不可读取。Codex CLI 当前没有可验证的全工具关闭模式，因此不得承载 screenplay，选择 Codex 时以 `analysis_cli_unsupported` fail closed。
...
剧本任务另受默认关闭的 `SCREENPLAY_ANALYSIS_ENABLED` 控制。该开关与 `ANALYSIS_ENABLED` 相与：视频 Worker 尚未完成按 `input_kind` 分派时，公开契约可以先部署，但创建剧本任务必须返回 503，不得把 screenplay 消息交给旧视频执行路径；只有受限剧本执行器和真实 E2E 通过后才允许开启。
