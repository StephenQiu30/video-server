...
- `download_thumbnails` 以 download job 为归属保存本地视频首个可解码帧的私有封面对象；新导入在验证工作区内尽力生成，既有成功记录由 Import Worker 有界扫描并回填，封面失败不阻断 Artifact 晋升。

视频导入继续出现在下载历史和下载详情，来源显示为“本地视频上传”，封面经 owner-scoped `/api/downloads/{job_id}/thumbnail` 读取；不得伪装为某个平台已下载内容，也不写 Provider canary。
...
`<analysis-work>` 与视频分析共享仓库外隔离根目录；Worker 会拒绝任何上级含有 `AGENTS.md` 的配置，避免 Codex 在最小权限沙箱启动时读取任务目录之外的项目指令。
...
剧本分析在单次有效字符预算内且不超过 120 个源场景时使用一次结构化调用；超过任一单次预算后，父 Worker 按连续完整场景确定性分块，每块分别获得严格校验的场景级结果，再把已校验的分块 JSON 交给不含 `scenes` 字段的受控汇总调用生成全局结论。汇总输入优先保留完整分块结果；当完整 JSON 超过有界容量时，仅供汇总调用的副本精简文字和条目，同时保留每个源场景 ID 与关键场景状态，原始已校验结果留在父 Worker 内存中用于最终合并。精简副本仍超限时明确以 `analysis_resource_limit` 失败，不扩大上下文。最终由服务端按源顺序合并原始场景结果，并再次验证全部源 scene ID 恰好覆盖一次、引用均在白名单内后才允许发布；单个完整场景本身超过字符预算或块数超过上限时也明确失败。Provider Schema 只使用 Codex/Claude 共同支持的严格结构化输出子集，字符串长度、数组容量、非空、唯一性与跨场景完整性由领域 parser 负责；CLI 诊断流保持有界捕获，但成功调用只以承载结果的 stdout/结果文件是否完整判断，避免大输入回显导致有效报告被误拒绝。

直接模块接入后，源文本的有效单次预算为配置上限与“配置上限 + 8,000 字符 Skill 预留量 − 本次 Skill 指令长度 − 用户补充要求长度”的较小值。上游模块越多，完整场景分块越小；单个完整场景超过有效预算时明确失败，不通过扩大上下文或拆散场景掩盖资源限制。
...
characters[] { id, name, goal, conflict, arc }
scenes[] { id, source_scene_id, purpose, conflict, turn, pacing, findings[] }
dialogue_findings[] { id, title, description }
strengths[] / priority_revisions[]
```

服务端只验证逐场景结果完整、唯一且顺序与上传剧本一致，以及结果结构、大小和各类 ID 的唯一性。人物、结构、对白和建议是 AI 对上传文本的分析判断，不宣称已由引用 ID 证明。

`source_scene_id` 仅供服务端核对逐场景覆盖。它由上传剧本的场景标题生成，不是截图、原文摘录或已核实的事实依据。分析契约不含场景“证据”列表；网页和报告按原文顺序给逐场景章节编号，并提醒读者对照上传的原文核查 AI 判断。`strengths`、`priority_revisions` 可以为空；审稿意见必须写出文本表现、影响及修改目标，不能为凑数生成条目。切换当前契约时一次性整理现存剧本分析结果和报告，不保留旧格式读取分支；之后的新报告版本保持不可变。
...
description: 审阅完整剧本的结构、人物、场景、节奏、对白与可执行修改建议。
license: MIT
metadata:
  video-server-display-name: 剧本故事审稿
  video-server-default-prompt: 先找主要问题与有效机制，再审阅结构、人物、场景与对白。
...
| Skill | 输入 | 结果契约 | 作用 |
| --- | --- | --- | --- |
| `screenplay-analysis` | screenplay | screenplay-analysis | 两个上游 Skill 库的七模块综合故事审稿 |
| `screenplay-drama-review` | screenplay | screenplay-analysis | 短剧故事承诺、局部回报与重复机制 |
| `screenplay-character-review` | screenplay | screenplay-analysis | 人物目标、冲突、策略与选择 |
| `screenplay-scene-review` | screenplay | screenplay-analysis | 逐场功能、节拍和状态变化 |
| `screenplay-dialogue-review` | screenplay | screenplay-analysis | 对白目的、策略和人物声音 |
| `screenplay-structure-review` | screenplay | screenplay-analysis | 聚焦节拍、场景推进、转折、连续性和节奏 |
| `screenplay-continuity-review` | screenplay | screenplay-analysis | 跨场人物知情、物件状态与时空因果 |
| `screenplay-rewrite` | screenplay | screenplay-rewrite | 中文/英文改写、本地化和同语言润色 |

视频 Skill 也使用同一 frontmatter 与结果契约声明；不保留第二套 legacy parser。

### 9.2 外部来源治理

- 生产环境和 API 不提供安装、上传或 URL Skill。
- 外部内容先审查名称、指令、引用、脚本、许可证和 Prompt Injection。允许把经审查的原始 Markdown 以固定版本直接放入仓库 `skills/modules/`，由项目 Skill 指定实际加载章节，并用本地适配指令约束其输出。
- 仓库记录来源 URL、固定 commit、许可证、NOTICE 和本地修改说明；升级必须重新评审和跑真实 fixture。
- Registry 读取 `SKILL.md`、显式允许的一层 `references/*.md` 和固定模块清单中的上游 Markdown 章节；模块文件的路径、哈希和标题均须匹配。拒绝符号链接、绝对路径、目录穿越、未知模块、未知结果契约和未知输入类型。
...
当前文档详情先展示文档 metadata 摘要，再以“正文阅读区 + 标题目录”两栏展示规范化 Markdown；阅读区固定高度并在内部滚动，文档信息不再占用目录侧栏。之后以一条全宽分隔线进入“剧本分析与改写”工作区。配置器只读取 screenplay Skill，沿用 `zh-CN/en-US`、4,000 字符补充要求和幂等创建；提交前持续披露规范化正文、任务指令、glossary 与有界上下文会发送给活动云端模型，并说明执行器没有文件、Shell、网络、浏览器、插件或其他 Agent 权限。任务状态复用 Analysis WebSocket、降级轮询、取消、原任务重试、删除与报告可用性语义。任务 Skill 快照与当前目录版本不一致时拒绝旧任务重试，并允许使用最新 Skill 创建新任务，保留旧任务记录。

成功的 `screenplay-analysis` 先展示“优先修改”和“值得保留”，然后呈现故事概览；结构、人物、对白、场景和完整报告进入键盘 Tabs。逐场景意见每页 20 场，按原文顺序展开阅读，不展示内部场景 ID，也不把 ID 伪装成截图证据。`screenplay-rewrite` 以源/输出场景统计、语义表格 glossary、修改摘要和 canonical Markdown 改写正文组织，不从公开 result 拼装第二份全文。两类结果只在报告同时具备 Markdown/DOCX Artifact 时显示导出动作。
...
| `analysis_input_expired` | 输入已删除，不能重新分析 | 否 |
| `analysis_skill_outdated` | 旧任务保存的 Skill 与当前版本不一致，需创建新任务 | 否 |
...
- 两个剧本结果 Schema、逐场景覆盖、chunk 覆盖、glossary 一致性和输出上限；
...
同一 Python 镜像继续承载 API、无 OAuth Worker 与 Runner；公共镜像/重启/健康检查骨架使用 YAML extension anchor 收敛，服务自身仍显式保留 `container_name`、网络、命令、权限和依赖。PostgreSQL、RabbitMQ、Redis、MinIO 直接复用本机已有服务；业务 Compose 读取现有 `.env` 的连接地址，不启动或重新初始化基础环境，不引入 SQLite 或第二套数据库过渡层。
