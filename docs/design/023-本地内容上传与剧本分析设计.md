...
同一 Python 镜像继续承载 API、无 OAuth Worker 与 Runner；公共镜像/重启/健康检查骨架使用 YAML extension anchor 收敛，服务自身仍显式保留 `container_name`、网络、命令、权限和依赖。PostgreSQL、RabbitMQ、Valkey/Redis、MinIO 直接复用本机已有服务；业务 Compose 读取现有 `.env` 的连接地址，不启动或重新初始化基础环境，不引入 SQLite 或第二套数据库过渡层。
