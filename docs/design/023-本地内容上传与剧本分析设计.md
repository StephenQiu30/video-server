...
6. 任一块或合并校验失败时整个任务失败，不保存部分成功报告；重试重新执行同一不可变输入、Skill、语言和确定性分块计划，不复用跨 attempt/run 临时输出。

当前默认限制为：完整剧本 glossary 输入 120,000 字符、单块 8,000 字符、最多 128 块、块前后各 1,000 字符上下文、全部改写正文合计 400,000 字符。父 Worker 在全部 glossary、块身份、覆盖、术语和总量校验通过后才返回一个 `ScreenplayRewriteResult` 给公共 validating/publish 阶段。
