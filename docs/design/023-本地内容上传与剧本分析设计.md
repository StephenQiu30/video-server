...
Analysis 联合输入的数据库形态已完成：现有任务回填为 video，SQL 约束拒绝双输入、无输入和 kind/contract/外键错配。当前 repository 已实现 screenplay 输入投影和事务内重校验；Document 与 normalized Artifact 必须同时满足 owner、ready、未删除、未过期和锁定 SHA-256 一致，创建/重试写入 Document 锁，所有既有终态与报告完成路径通过统一输入锁释放函数处理。剧本 Analysis API、独立 screenplay 执行器及报告链均已公开，且不允许剧本回落到视频执行器。
