...
Analysis 联合输入的数据库形态已完成：现有任务回填为 video，SQL 约束拒绝双输入、无输入和 kind/contract/外键错配。当前 repository 已进一步实现 screenplay 输入投影和事务内重校验；Document 与 normalized Artifact 必须同时满足 owner、ready、未删除、未过期和锁定 SHA-256 一致，创建/重试写入 Document 锁，所有既有终态与报告完成路径通过统一输入锁释放函数处理。该持久化能力尚未公开剧本 Analysis API，也未让现有视频执行器读取文档。
...
当前已落地的分析与改写路径权限更窄：只支持 Claude CLI 的 `--tools ""` 单轮结构化调用，规范化剧本、改写源块、相邻上下文和 glossary 都以 JSON 数据嵌入标准输入，CLI 不获得 Read、Write、Edit、Bash、网络、FFmpeg、MCP、插件、浏览器、Agent 或 subagent。工作区中的 `input/screenplay.md` 仅供父 Worker 完整性复核；确定性 chunks 和已解析输出只保存在父 Worker 内存中，模型不可读取文件。Codex CLI 当前没有可验证的全工具关闭模式，因此不得承载 screenplay，选择 Codex 时以 `analysis_cli_unsupported` fail closed。

剧本分析在容量允许时使用一次结构化调用；`ANALYSIS_SCREENPLAY_SINGLE_CALL_CHARACTERS` 默认限制为 120,000 字符，且当前结果最多覆盖 120 个源场景。Schema 序列化后另设 28,000 字节硬上限，避免 Windows 子进程命令行因场景证据枚举膨胀。超过任一上限先以 `analysis_resource_limit` 终止且不下载、不调用模型；在父 Worker 的确定性分块实现和验收完成后，再按场景生成只读 chunks，先做场景级结果，再用只包含场景摘要和证据 ID 的第二次调用生成全局结论，不允许退化为无界上下文。
...
当前默认限制为：完整剧本 glossary 输入 120,000 字符、单块 8,000 字符、最多 128 块、块前后各 1,000 字符上下文、每块最多 2 次调用且首次恢复等待 1 秒、全部改写正文合计 400,000 字符。块内恢复期间仍由同一个 lease monitor 维持心跳；父 Worker 在全部 glossary、块身份、覆盖、术语和总量校验通过后才返回一个 `ScreenplayRewriteResult` 给公共 validating/publish 阶段。
...
剧本任务另受默认关闭的 `SCREENPLAY_ANALYSIS_ENABLED` 控制。该开关与 `ANALYSIS_ENABLED` 相与；开启后 Worker 会在连接队列前同时预检 `screenplay-analysis` 与 `screenplay-rewrite` 能力，任一能力缺失即 fail closed。当前受限执行链已实现，但只有 Claude 中文/英文真实 E2E、报告与浏览器流程通过后才允许在正式环境开启。
...
当前文档详情在规范化正文/metadata 两栏之后，以一条全宽分隔线进入“剧本分析与改写”工作区。配置器只读取 screenplay Skill，沿用 `zh-CN/en-US`、4,000 字符补充要求和幂等创建；提交前持续披露规范化正文、任务指令、glossary 与有界上下文会发送给活动云端模型，并说明执行器没有文件、Shell、网络、浏览器、插件或其他 Agent 权限。任务状态复用 Analysis WebSocket、降级轮询、取消、原任务重试、删除与报告过期语义。
...
Compose 只表达容器拓扑需要的服务角色、连接地址、跨进程 Secret、容器固定路径和显式能力开关。大小、TTL、GC 周期、批量、租约、解析器预算等已有类型化安全默认值的运行参数只在 `backend/app/core/config.py` 维护，不在基础或生产 Compose 重复展开。生产覆盖继续在解析阶段要求关键 Secret 和连接地址；RabbitMQ 的生产 queue type 属于基础设施拓扑策略，因生产值与初始化脚本默认值不同而保留显式覆盖。

同一 Python 镜像继续承载 API、无 OAuth Worker 与 Runner；公共镜像/重启/健康检查骨架使用 YAML extension anchor 收敛，服务自身仍显式保留 `container_name`、网络、命令、权限和依赖。PostgreSQL、RabbitMQ、Valkey、MinIO 与初始化任务继续只属于可选 `environment` Profile，本机已有 PostgreSQL 时不得再引入 SQLite 或第二套数据库过渡层。
