...
| `/providers` | `app/providers/page.tsx` | 已登录 | 筛选平台支持状态，并按需展开验证证据 |
| `/account` | `app/account/page.tsx` | 已登录 | 查看邮箱与身份、修改用户名 |
| `/admin/files` | `app/admin/files/page.tsx` | 管理员 | 分页查看持久文件并清理历史资产 |
...
- `BasicLayout` 是全部路由唯一的页面外壳，统一提供 Header、`.content-shell` main、Footer、跳过链接与站内导航历史；公开页、认证页、业务页和管理页不得绕过它创建第二套 Header、main 或根容器。Header、main、footer 与认证内容统一使用 `.content-shell = min(calc(100% - 160px), 1376px)`，让品牌、导航操作、欢迎页 Hero 和认证双栏共享左右对齐线。`body` 作为始终可见的纵向滚动容器，稳定长短页面切换；页面根节点不额外设置 `scrollbar-gutter`，滚动锁与滚动条宽度补偿统一交给 Radix 覆盖层原语。Dropdown、Dialog、Select 和 Sheet 不得再引入第二套全局补偿或覆盖 `data-scroll-locked`，避免同一滚动条被重复计算而让整页抖动。Header 内的认证账户区域使用固定宽度槽位，加载占位、登录入口与 Avatar 菜单切换时不得重排其他导航项。认证页只在共享 main 内保留无外框双栏内容，左侧产品主张复用欢迎页 `.editorial-title` 尺度，右侧表单在网格内收窄到 440px，不足 `lg` 时隐藏介绍栏。在 641–1023px，`.content-shell` 改为两侧各 32px gutter；在 `<=640px` 时改为两侧各 16px gutter。这是对齐约束，不得呈现为可见页面外壳；资料表单、说明文字和媒体可在对应网格内二次收窄。
...
2. 主输入以 Field + InputGroup + Button 组合，InputGroup 使用浅中性实心填充，解析按钮使用近黑填充；两者都不使用装饰性边框。输入有真实 label、Phosphor 链接图标、清除动作、提交中状态和内联错误。链接输入与主按钮在移动端统一为 64px、`sm` 及以上统一为 68px，不能因透明边框或内容自适应多出 2px；图标、占位文案、单行链接和按钮内容必须共享垂直中心。输入完整平台分享文案时允许两行自然居中，超过可视高度后在控件内部滚动，不得撑开操作行或挤压主按钮。三个内容入口共用 `IntakeControlRow`、选择按钮与提交按钮组件，业务页面不得分别维护同一组高度和网格常量。
3. 旧的“链接 → 格式 → 下载”三步导航/进度 UI 不再出现；页面用留白、操作状态和结果内容的自然顺序表达流程。链接入口仅输入框和解析按钮，不展示访问策略、会话、线路证据或刷新策略操作，不为这些内容订阅平台状态；运行选择与诊断由服务端及管理端承担。
...
7. 未登录首页保留产品能力、自托管架构、注册与源码入口，但必须与已登录首页复用 `EditorialIntro`、`.editorial-title`、`.content-shell` 和 Button 表面。匿名与已登录首屏统一使用“把素材，带回本地。”，不能另建更大的营销标题尺度；产品能力、架构与本地启动区只靠网格、排版和 80/112px 区段留白建立层级，不显示区域横线、双栏竖线、能力列分隔线或独立 Card 外壳。
...
认证页与其他路由一样由 `BasicLayout` 提供同一个 Header、main 和 Footer；`AuthPageFrame` 只是共享 main 内部的 `1.15fr / 0.85fr` 无外框双栏内容，不再渲染独立品牌栏、主题栏、main 或 `.content-shell`。桌面左栏承载复用 `.editorial-title` 的产品主张，右栏通过相同 96px 列间距放置不超过 440px 的表单，不使用竖向分隔；品牌入口、主题按钮、双栏起点与欢迎页处在同一组水平对齐线上。不足 `lg` 时隐藏介绍栏，并在可用内容区域内水平居中表单，手机宽度下再自然占满可用宽度。登录突出邮箱、密码和单一近黑主按钮；注册增加用户名与密码确认。标题使用“登录，继续下载。”或“创建账户，保存进度。”等直接动作句，不增加编号流程眉题。字段统一由 Field + InputGroup 组合 label、描述、密码可见性与错误，错误出现在对应字段附近并在提交失败时聚焦摘要。登录态用户访问认证页时返回校验后的安全站内目标；登录和注册通过明确的交叉链接互相切换，不显示可能回到过期受保护页面的通用历史返回。
...
- 封面与内联播放器在同一断点和位置统一宽度，高度由各自素材的真实比例自适应。横图、竖图、方图均等比铺满可用宽度，完整展示，不设固定媒体比例、不拉伸、不裁切、不以固定外框补边强行等高。加载或缺失尺寸时使用可读的临时占位，图片解码或视频元数据就绪后允许高度自然变化；长竖图通过页面滚动查看。全屏仍按设备视口容纳完整画面。长标题最多按上下文截断并保留完整可访问名称。
...
2026-09-12 恢复交互：下载列表/详情按 `source_kind` 区分恢复方式；本地导入失败、取消或制品缺失时明确返回首页重新选文件，远程来源保留下载重试。无封面占位的辅助文字复用 `background/70`，不可降低到浅色主题下未达 4.5:1 的 `background/45`；不改变全局颜色 token、排版或布局。

## 邮箱验证注册（2026-09-08）

注册表单在邮箱字段之后、密码字段之前放置六位验证码与获取按钮，复用 AuthField、InputGroupInput 和 Button。发送状态通过文本播报，重发冷却使用服务端返回值；邮箱编辑后清空验证码。继续使用原有双栏/移动单栏、无边框布局与明暗主题，允许表单纵向滚动。业务契约与验收见 [033](033-邮箱验证注册设计.md)。
