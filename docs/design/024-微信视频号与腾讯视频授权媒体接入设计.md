...
现有 `qqvideo-public-video` 不能继续静态标为 `verified`。固定 yt-dlp Tencent extractor 会以硬编码参数生成消费端 `cKey`、调用未公开播放接口并请求 DRM 能力，但结果没有可靠的 `public/free/VIP/purchase/preview` 权益证据。这与本设计“不生成消费端私有签名”和“公开 clear 必须有正向证明”的边界冲突。
