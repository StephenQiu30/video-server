...
> 当前范围：024 维护公众号发现与官方 Connector；腾讯个人单视频下载的产品范围、会话和发布证据以 [032 设计](032-腾讯视频与优酷个人下载设计.md) 为准。
> 2026-08-27 修订：视频号公开 `/sph/` 单视频链接的当前实现与安全边界已由 [025 设计](025-微信视频号公开分享链接下载设计.md) 取代。本文件中“视频号链接只返回 export_required/unsupported”的旧结论仅保留为历史背景；公众号文章与腾讯云 VOD 部分仍有效，腾讯个人消费站范围由 032 维护。
...
- 公众号与官方 Connector 不接收微信、元宝或腾讯消费端会话；032 的腾讯个人会话仅在独立文件来源 Runner 使用。
...
保留 `wechat-channels-public-share` 研究位，但默认不注册。只有同时满足以下条件才允许实现：
...
腾讯与优酷个人会话路径由 [032 设计](032-腾讯视频与优酷个人下载设计.md) 维护。当前腾讯单视频入口按配置启用，真实 VIP 完整下载尚未验收；公众号发现的腾讯嵌入仍按独立发现流程处理，不由本变更自动下载。
...
```json
{
  ...
  "connector_version": "tencent-vod-owner",
...
1. 立即把 `qqvideo-public-video` 从静态 `verified` 降为 `unknown/disabled`，在生产阻止当前 Tencent extractor 生成下载格式。
...
| 领域 | 主要落点 |
| --- | --- |
| 来源/决策领域 | `backend/app/services/provider_types.py`、新增 `domain/media_sources.py` |
| API/Schema | `backend/app/api/routes/inspections.py`、新增 `routes/source_discoveries.py`/`provider_connections.py`、对应 `schemas` |
| 解析编排 | `backend/app/services/downloads/`、`integrations/media_inspection_pipeline.py` |
| 公众号文章 | 新增隔离的 `integrations/article_discovery/` 静态 Adapter 与 fixtures，不放入 Generic/浏览器自动化目录 |
| 权益/保护 | `backend/app/workers/runner/entitlements.py`、新增共享 `media_protection.py` |
| 腾讯公开视频 | `backend/app/workers/runner/provider_catalog_public.py`、Provider normalizer/error/canary tests |
| 官方连接器 | 新增 `backend/app/integrations/official_connectors/`，不放入 yt-dlp plugin 目录 |
| 下载路由 | `backend/app/services/download_execution/`、`integrations/media_runner.py` 的上层 Router |
| 导入验证 | `backend/app/integrations/imports/video.py` 复用 ProtectionClassifier |
| 持久化 | `backend/sql/schema.sql`、`models` 和 repository |
