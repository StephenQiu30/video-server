...
> 本设计是微信公众号文章、微信视频号和腾讯受限媒体接入的当前规范。它取代 010 调研和 019 设计中关于“元宝 Cookie、私有 finder 接口、微信根证书代理、页面注入、`decodeKey`、受保护前缀转换”的产品化建议。019 的设备配对、签名上传和 Artifact Import 思路仍可复用，但 Edge Agent 只能传输用户已经合法取得的明文文件，不能承担拦截、解密或扩大平台权益的职责。
...
- `qqvideo` 的目标能力只处理 `v.qq.com` 公开、clear 单视频；当前代码仍将固定 yt-dlp Profile 静态标为 `verified` 并运行 canary，但该 extractor 使用未公开播放接口、生成消费端 `cKey`，又缺少公开免费正向权益证据，所以 Phase 0 必须先降为 `unknown/disabled`，完成 Phase 2 门禁后才能恢复；
