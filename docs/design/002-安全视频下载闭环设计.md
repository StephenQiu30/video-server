...
首页使用 Ant Design Pro 原生 `PageContainer + ProForm`：页面标题由 `PageContainer` 提供，URL 输入、必填校验与提交按钮由 `ProForm` 管理。未开始解析时不展示结果容器；仅在解析中、失败或已有解析结果时渲染对应状态。页面不再维护营销式 Hero、英文眉题、平台彩色图例或输入框组合样式。
