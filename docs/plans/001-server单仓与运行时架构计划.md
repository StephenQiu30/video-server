...
- 根目录三份 Compose 按职责分层：基础文件负责公共拓扑，`-env` 负责本地环境差异，`-prod` 负责生产环境差异。
- 本地使用 `docker-compose.yml` 叠加 `docker-compose-env.yml`，生产使用 `docker-compose.yml` 叠加 `docker-compose-prod.yml`；环境变量值不写入非 env 文件。
