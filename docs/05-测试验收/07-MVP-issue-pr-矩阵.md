...
- PR #31/#32/#33 当前已关闭，等价实现已通过 `main` 上的提交落地：
  - `e2b06ed`：服务器内置前端拆除与部署链路更新
  - `15aa5ed`：API-only 与平台适配器抽象
  - `197be62`：API 契约/任务/API 权限链路测试补齐
  - `d78ad91`：测试契约与 `TaskRead` 对齐修正
- 对应 issue 仍可按 `#1-#30` 的归档状态关闭（已闭环）。
