...
- 配置收敛：镜像、宿主机绑定、容器内部服务名、队列、网络和卷由 Compose 提供默认值；`.env.example` 只保留宿主机进程可能需要覆盖的依赖连接和可选 Provider 凭据，`.env.prod.example` 保留生产镜像、基础设施凭据、API/Runner 密钥、公共对象地址与 Provider Key。
