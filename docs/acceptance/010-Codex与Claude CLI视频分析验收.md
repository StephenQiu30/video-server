...
| 分镜领域校验 | Passed | 严格拒绝未知字段、非法类型、重复 ID、空/悬空证据、间隙、重叠、越界和非法枚举；当前契约另区分物理编辑与连续视觉节拍，并要求长视频单项结果显式复核 |
...
- 需要增加一个无物理 Cut、但包含多个空间/动作/信息阶段的长镜头 fixture，验证输出使用 `transition_in=continuous` 而不是退化为整片单一分镜。
