...
- 本文描述目标验收；当前代码尚未实现受控 Cookie/POT，所有新增项保持未勾选。
...
- [ ] A1：文档、`AGENTS.md`、`SECURITY.md`、根/后端 README 对 Cookie 的表述统一为受控 Provider 会话，不再一刀切禁止。
- [ ] A2：产品继续明确仅处理用户有权访问的非 DRM 内容，Cookie 不扩大 private/会员/购买权益。
- [ ] A2a：YouTube 运维账号无 Premium、频道会员、购买/租赁、private share 或个人资产；账号权益漂移会自动 disable。
- [ ] A3：固定记录 yt-dlp、EJS、curl-cffi、POT Provider 和插件的 commit/version、许可证和 SBOM。
- [ ] A4：当前 YouTube 故障被复现并区分为 EJS、credential、POT、egress challenge 或 provider regression，而不是泛化 502。
- [ ] A5：`POST /api/inspections` 直接携带 `cookie` 仍返回 422，且错误/日志不回显该值。
- [ ] A6：MeTube/cobalt/gallery-dl 参考代码没有被未经许可复制；公共解析服务没有接收用户 URL/凭据。

## 3. B. Provider Profile 与访问上下文

- [ ] B1：17 个 Registry key 有版本化 Profile、capability、access mode、Cookie 域、client、attestation、出口、并发、错误和 canary 配置。
- [ ] B2：registered host、extractor exists、verified、access required、unknown 和 unsupported 是不同状态。
- [ ] B3：Generic 永远没有 Cookie/POT；redirect 到已知 Provider 后重新校验和选择 context。
- [ ] B4：inspection 冻结 profile、access mode、credential version、egress affinity、client、POT Provider 和 engine commit 引用。
- [ ] B5：下载前 re-inspect、视频流、音频流、probe sample 与 remote probe 使用同一 credential snapshot/client/egress context；一次 download 内串行复用操作级 Cookie jar。
- [ ] B5a：初次 inspection 的临时 Cookie 更新不进入 DB/消息；download 仍从原 version 重建 jar 并重新 inspect，不替换为当前 active version。
- [ ] B6：人为修改 client/credential/egress/version 返回 `client_context_mismatch`，不静默回退或继续旧媒体 URL。
- [ ] B7：context 被撤销、版本不存在或 Profile disabled 时排队任务不启动。

## 4. C. Secret 隔离与生命周期

- [ ] C1：YouTube Cookie 源只读挂载到 credentialed Runner，API、Worker 和 anonymous Runner 不可见。
- [ ] C2：Cookie 内容不在环境变量；配置只含固定路径和非 Secret version id。
- [ ] C3：源文件验证普通文件、no symlink、Netscape header、≤1 MiB 和 YouTube 域 allowlist。
- [ ] C4：每次 Runner 操作在独占 tmpfs 使用唯一目录 `0700` 和 Cookie jar `0600`；同一 jar 没有并发 writer。
- [ ] C5：临时副本不在 Runner/Download Worker 共享 `/work`，Worker 无法读取。
- [ ] C6：成功、子进程失败、timeout、cancel、SIGTERM、workspace limit 和 Runner restart 后均无临时副本残留。
- [ ] C7：并发操作的路径/inode 不同；一次 download 的后续命令能看到该 jar 的更新，且不改变只读源、其他任务或下一次操作。
...
- [ ] C9：非 YouTube 与 Generic 命令不含 `--cookies`，也无法打开 YouTube Secret。
- [ ] C10：Runner 仍不能访问 DB/MQ/MinIO/Valkey、Docker socket 或绕过 egress proxy。
...
- [ ] D4：受会话保护的媒体 URL 在 remote probe 阶段不会因丢失 Cookie/Referer/UA 而失败。
- [ ] D5：有效 Cookie、缺失 Cookie、过期/rotated Cookie、拒绝 Cookie 和撤销 Cookie 得到稳定且不同结果。
- [ ] D6：每个运维 credential subject 实际 active job 不超过 1；并发竞争不超发。
...
- [ ] D10：credentialed inspect/re-inspect 只允许明确 public/unlisted 且不依赖账号权益；private、premium、subscriber、needs_auth、unknown 和 DRM 在任何媒体字节下载前终止。
- [ ] D11：权益拒绝不会创建 stream 文件、artifact、MinIO 对象或下载记录；canary 发现账号权益漂移会 disable version。

## 6. E. POT、出口与重试

- [ ] E1：POT sidecar 仅向 YouTube credentialed Runner 暴露，版本和许可证进入 SBOM/NOTICE。
...
- [ ] E7：凭据、DRM、权益、geo、deleted 和 unsupported 默认不重试。
...
- [ ] F2：download re-inspect 的 Provider 错误不再归为 `worker_lost` 或触发基础设施重试。
- [ ] F3：当前“cookie uploads are not supported”文案已移除，用户看到具体、合法且不泄密的动作。
- [ ] F4：公开 Problem Details 不暴露 Cookie 是否存在、账号、version、POT、出口地址、命令或临时路径。
- [ ] F5：`media_url_expired` 只用原 context 重检一次；仍失败则稳定终止。
- [ ] F6：DRM、private、会员/购买无权益为终态，不提示继续上传/更换 Cookie 规避。
- [ ] F7：Provider 状态页不使用静态清单或“1000+”承诺实时成功。
...
- [ ] G6：Bilibili 公开 UGC 回归成功，命令不含 Cookie/POT。
- [ ] G7：抖音公开分享页回归成功；动态签名/schema 故障不伪报 Cookie 缺失。
- [ ] G8：小红书有效公开分享链回归成功；短链失效、图文笔记、原画缺失分别分类。
- [ ] G9：TikTok、Vimeo、X、Instagram、Facebook、Twitch、Reddit 在显示 verified 前完成 metadata + media canary。
- [ ] G10：Pinterest、微博、优酷、腾讯视频、Dailymotion、NicoNico 无真实证据时保持 unknown。
- [ ] G11：视频号、快手稳定为 unsupported，Generic 不会把它们提升为 supported。
...
- [ ] H6：production-like Compose 证明 Runner pool、Secret mount、tmpfs、网络和 kill switch 符合设计。
...
截至 2026-08-10，当前代码仍没有 Cookie/POT 会话能力，用户截图中的 YouTube 失败可复现，目标方案只完成研究与设计，尚无实施或真实验收证据。Phase 1 与 Phase 2 均保持 Pending，不得宣称 YouTube 或注册表中的全部平台已受控支持。
