...
请求执行前先持久化 `started` 记录。初始写入失败时不执行请求。已认证账户作为操作人快照写入结果，日志只完成一次；最终落库失败时保留“结果未确认”并记录 `operation_log_finalization_failed` 与日志 ID，不把已执行业务操作自动重放。中断留下的记录不能作为任务仍在运行的证据。
