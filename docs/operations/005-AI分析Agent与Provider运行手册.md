...
若本机使用仓库基础环境且 `doctor` 返回 MinIO readiness probe 缺失，在 `backend` 目录执行 `docker compose --env-file ../.env -f ../docker-compose-env.yml run --rm minio-init`，由环境初始化任务幂等创建 `video-artifacts` bucket 与 `system/analysis-readiness-v1` 探针。外部或生产 MinIO 应由其管理员创建同名探针，不能另起一套本机 MinIO 掩盖连接错误。若返回凭据无法读取，再确认统一 AK/SK 的 bucket 读取权限，并检查 `.env` 或 `.env.prod` 中唯一的 `MINIO_ACCESS_KEY` / `MINIO_SECRET_KEY`。
