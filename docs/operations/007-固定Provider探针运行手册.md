...
未知 `--provider` 返回 2，且不创建 runtime、不执行部分已知平台；避免拼写错误造成漏测而误报通过。先对齐整组镜像和 schema，生产文件来源按[重启换机手册](008-个人部署重启与换机手册.md)准备。诊断保持串行，时长/大小采用预先声明的 Runner 预算；超时或超限记为未在预算内完成，不能临时降低清晰度或换样本。固定样本不是所有内容或所有能力的发布保证。
