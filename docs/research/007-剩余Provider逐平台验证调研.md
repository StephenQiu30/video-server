...
| 腾讯视频 | 历史媒体探针通过；024 重新判定不满足权益/接口门禁 | 约 216 秒，音视频轨道通过 | 当前运行时为 `disabled` | 历史样本只能证明当时取得媒体；未公开 `cKey` 机制和缺少 `public_free` 正向证据，不能作为当前生产批准；024 Phase 0 已实施 |
...
本轮在 2026-08-12 历史上新增六个严格限定的匿名公开单视频/Clip Profile；024 复核后只有 Facebook、Twitch、Pinterest、微博和优酷的这批证据继续有效。腾讯视频已在 Phase 0 降为 `disabled`；YouTube 与 Reddit 保持 `access_required`；TikTok 已改为 anonymous-only Player v3，旧网页/operator 证据不再参与当前状态。这不是对平台全部内容类型的承诺，也不改变 DRM、付费、会员、私密、地域限制和多资产内容的拒绝策略。
