...
异步下载主链路的第一步是把"接单"和"查单"稳定下来。当前实现已存在于 `apps/api/app/routers/tasks.py` 和 `apps/api/app/services/tasks.py`，包含完整的任务创建、列表查询、详情查询接口。本次变更的目标是将已有实现的行为契约通过 OpenSpec 规范化。
