...
### Requirement: Task State Model Must Have Six States
The system MUST define exactly six task states: `queued`, `running`, `succeeded`, `failed`, `canceled`, `expired`.
...
- **THEN** it contains exactly: `QUEUED`, `RUNNING`, `SUCCEEDED`, `FAILED`, `CANCELED`, `EXPIRED`
- **AND** the string values are `queued`, `running`, `succeeded`, `failed`, `canceled`, `expired`

#### Scenario: Expired state represents artifact cleanup
- **GIVEN** a task that previously succeeded
- **WHEN** its artifacts expire and are cleaned up
- **THEN** the task state transitions from `succeeded` to `expired`
- **AND** the `object_key` is cleared
...
| `running` | `succeeded` | Download + upload complete |
| `running` | `failed` | Download, probe, or upload error |
| `running` | `canceled` | User cancels |
| `succeeded` | `expired` | Artifact retention period ends |
...
Retrying a failed, canceled, or expired task creates a new task with `retry_of_task_id` pointing to the original.
...
#### Scenario: Retry an expired task
- **GIVEN** a task in `expired` state
- **WHEN** the user requests retry
...
- **GIVEN** a task whose artifacts have expired
- **WHEN** `GET /api/tasks/{task_id}/download-link` is called
