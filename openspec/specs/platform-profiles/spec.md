...
所有平台的 `compliance_note` 必须遵守以下规则：
...
任务详情接口应返回与 ParseResponse 一致的平台字段，确保前端可统一展示。
...
### 7.1 当前实现（apps/api/app/services/platforms.py）

已注册 10 个平台画像，其中包含 PRD02 要求的 YouTube、Bilibili、TikTok。

### 7.2 差距

| 差距项 | 说明 | 归属 |
| --- | --- | --- |
| 缺少 X 平台 | PRD02 要求但实现未包含 | 下游实现 ticket |
| 缺少 Instagram 平台 | PRD02 要求但实现未包含 | 下游实现 ticket |
| ParseResponse 缺少 platform_display_name | schemas.py 未定义该字段 | 下游实现 ticket |
| TaskRead 无平台字段 | schemas.py TaskRead 未包含平台字段 | 下游实现 ticket |
| category 命名不一致 | PRD02 用 `overseas-video`，实现用 `long_video` | 下游实现 ticket |
