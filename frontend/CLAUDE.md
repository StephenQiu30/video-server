...
- 所有路由复用两级隐形网格：72px Header 使用 `.page-shell = min(calc(100% - 80px), 1456px)`，常规 main/footer 使用 `.content-shell = min(calc(100% - 160px), 1376px)`；认证双栏 main 是唯一例外，可使用 `.page-shell`，右侧表单在内部收窄到 440px，不足 `lg` 时隐藏介绍栏。641–1023px 时内容区两侧各 32px，不超过 640px 时两级网格两侧各 16px。网格只提供对齐，不得呈现为可见外框。
...
- 图标统一使用 `@phosphor-icons/react`，不使用 emoji、文本符号、手写 SVG、CSS 图形或第二套图标库代替产品图标。
