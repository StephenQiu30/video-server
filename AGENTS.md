...
- 系统技术架构与核心规格统一在 `docs/design/` 维护，部署与运维指导在 `docs/operations/` 维护。
- 开发过程中的临时中间状态文件（需求 PRD、实施计划 Plan、阶段验收 Acceptance、背景调研 Research）在合并后不长期保留，已合入设计、代码或手册的纯过程文件及时清理并通过 Git 历史追溯。
