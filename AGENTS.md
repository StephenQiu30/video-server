...
- SDD 文档按职责分开：`docs/prd/` 是产品范围、FR／NFR／AC 的唯一入口；`docs/design/` 维护技术方案；`docs/plan/` 维护执行步骤、依赖、状态及证据；`docs/operations/` 维护运行指导。BACKLOG 提供项目导航和历史记录，不复制当前 Plan 的状态。
- 有效 PRD、Design、Plan 作为 SDD 开发规格持续维护，不因文档提交或部分代码合并而删除。需求／验收编号保持稳定，变更同步技术设计、执行任务与测试；只有被替代或已失效的纯过程稿通过 Git 历史追溯，禁止维护多份相同的当前事实。
