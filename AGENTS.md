...
12. 正式项目文档写入 `docs/` 的既有分类并遵循 `AGENTS.local.md`；临时计划、日志、prompt、缓存和一次性状态文件不得提交。
...
5. 用户界面任务优先用可执行的端到端测试、截图、trace 或录屏证明关键路径。
...
3. `Agent Review` 阶段使用 `reviewer:codex` 标签。
4. 默认 reviewer 为 `reviewer:codex`；不要使用旧式 `review:*` 标签。
...
6. 新分支必须匹配 `^feature/[a-z][a-z0-9_]*$`，例如 `feature/login_validation`；分支名不含 ticket ID、中文、空格或连字符。
