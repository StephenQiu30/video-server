...
- 前端页面默认以 Ant Design Pro / Umi Max 脚手架结构为基线，不得用手写页面外壳替代脚手架布局。
- 页面容器、主内容宽度和页面间距优先交给 ProLayout、PageContainer、ProCard、ProForm、ProList、ProTable 等 ProComponents 管理。
- 不允许随意新增 `.page-shell`、`.workspace-container`、`.download-wrapper` 等自定义主页面尺寸容器；确需补充样式时，只能用于小范围兼容和交互状态。
- 下载工作台必须保持单列工具流，不恢复左右分栏、营销首页、合规说明主导航页或复杂 SaaS 视觉。
- 重新生成或大幅调整 `apps/web` 前，必须先提交当前稳定状态；重生成后必须验证链接解析、清晰度选择、任务进度、下载、重试和历史页不退化。
