...
## 角色协作结构
当前项目支持 Codex、Claude 和 Cursor 三套多 Agent 入口并存；不得把一套入口替换为另一套入口。Codex 侧按 `AGENTS.md + AGENTS.local.md + .codex/agents/*.toml` 的方式组织协作规则：
...
4. `CLAUDE.md + CLAUDE.local.md + .claude/agents/*.md + .claude/skills/*` 记录 Claude 侧对应规范。
